...
const ASSETS = ['index.html', 'core.js', 'routing.js', 'roadmeta-part-00.js', 'roadmeta-part-01.js', 'roadmeta-part-02.js', 'roadmeta-part-03.js', 'roadmeta-part-04.js', 'roadmeta-part-05.js', 'roadmeta-part-06.js', 'roadmeta-part-07.js', 'roadmeta-part-08.js', 'roadmeta-part-09.js', 'roadmeta-part-10.js', 'roadmeta-part-11.js', 'roadmeta-part-12.js', 'roadmeta.js', 'roadindex.js', 'roads.js', 'map.js', 'route-path.js', 'guidance.js', 'route-progress.js', 'route-trip.js', 'pwa.js', 'gps.js', 'data.js', 'basemap.js', 'ferry.js', 'manifest.webmanifest', 'build-info.json', 'ATTRIBUTION.txt', 'icon-192.png', 'icon-512.png'];
...
async function mapConcurrent(items, limit, work) {
  ...
  const workers = Array.from({ length: Math.min(limit, items.length) }, async () => {
    while (next < items.length) { const i = next++; await work(items[i]); }
  });
  await Promise.all(workers);
}
async function cacheAssets(refresh = false) {
  const cache = await caches.open(CACHE);
  await mapConcurrent(ASSETS, 4, async path => {
...
self.addEventListener('message', event => {
  ...
  if (event.data?.type === 'PREPARE_OFFLINE') {
    event.waitUntil(cacheAssets(false).then(reply).catch(async e => {
      const status = await cacheStatus().catch(() => ({ ready: false, missing: ASSETS }));
      reply({ ...status, type: 'CACHE_STATUS', error: e.message });
    }));
  }
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith((async () => {
      ...
      try {
        ...
        return new Response('NL Offline has not finished preparing its offline package. Reconnect once and press Prepare offline map.', { status: 503, headers: { 'Content-Type': 'text/plain' } });
      }
    })());
    return;
  }
